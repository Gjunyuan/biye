...
$sql = "SELECT * FROM `comment` WHERE `id`='$id' and `UserGive` LIKE '%$Sid%'";
...
if (mysqli_num_rows($abc)) {
  ...
  //上传数据
  $sql_usergive = "UPDATE `comment` SET `UserGive` = '$str' WHERE `comment`.`id` = '$id'";
  $abc_usergive = mysqli_query($con, $sql_usergive);
  //查询点赞数量
  $sql_give = "SELECT * FROM `comment` WHERE `id`='$id'";
  ...
  //上传数据
  $sql_usergive = "UPDATE `comment` SET `UserGive` = '$stri' WHERE `comment`.`id` = '$id'";
  $abc_usergive = mysqli_query($con, $sql_usergive);
  //查询点赞数量
  $sql_give = "SELECT * FROM `comment` WHERE `id`='$id'";
  $abc_give = mysqli_query($con, $sql_give);
